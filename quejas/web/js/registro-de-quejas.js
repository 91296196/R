...
const agregar_aclaracion = ()=>{
    ...
    console.log(dataJSON.get("numero_de_folio"))
...
const ver_detalle = ()=>{
  ...
  for (let i = 0; i < opciones.length; i++) {
    if (opciones[i].checked) {
      ...
      let txtObservaciones=document.getElementById('observacion')
      let lbObservacion=document.getElementById('lb_observacion')
      ...
      txtObservaciones.classList.add('d-none')
      lbObservacion.classList.add('d-none')
...
const ver_Facturas = () => {
  ...
  txtObservaciones.classList.remove('d-none')
  cardDetalle.classList.add('d-none')
  lbObservacion.classList.remove('d-none')
} 
